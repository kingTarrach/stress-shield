...
struct CheckInView: View {
    ...
    @State var questions: [(question: String, options: [String])] = [
        ...
        VStack {
            if !isLoaded {
                ProgressView("Loading Check-In...")
                    .onAppear {
                        Task {
                            await self.ViewModel.getGoalQuestions()
                            print(ViewModel.userGoalQuestions)
                            questions.append(contentsOf: ViewModel.userGoalQuestions)
                            self.isLoaded = true
                        }
                    }
            }
                    ...
                    HStack {
                        ...
                        Button(action: {
                            ...
                            Text(stepIndex < questions.count - 1 ? "CONTINUE" : "FINISH")
                                ...
                                .cornerRadius(10)
                                .opacity(responses["\(stepIndex)"] == nil || responses["\(stepIndex)"] == "" ? 0.5 : 1.0)
                        }
                        .disabled(responses["\(stepIndex)"] == nil || responses["\(stepIndex)"] == "")
                    }
                    .padding()
                }
    ...
    private func saveCheckInData() {
        ...
        Task {
            await ViewModel.processCheckIn(responses: responses, HRV: 0.0, Sleep: 0.0)
        }
